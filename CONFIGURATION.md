...
| `budgets.initMs` | `number` | `200` | Sub-budget (ms) for the MCP-ready wait stage of `before_prompt_build`. If exceeded, a warn log is emitted and an `init.overrun` counter is incremented, but execution continues. Overruns are advisory latency telemetry and do not by themselves degrade `/remempalace status`. The total 1500 ms deadline always takes precedence. |
